...
void Threaded(string selector,string phrase, string shift,string key, string version ){
  if(selector == "caesar"){
    std::thread th1(caesar,phrase,shift,version);
    th1.join();
    ...
    std::thread th2(rot13,phrase,version);
    th2.join();
    ...
    std::thread th3(vig,phrase,key,version);
    th3.join();
    //cout << "done with vig" << "\n";
  }
